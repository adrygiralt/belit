...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const result = await executeQuery({
    query: `SELECT pl.id,
            ...
            WHERE j.jugador = pl.num AND j.partit = p.id AND (p.equip1 = pl.equip AND j.num IN (1,2,3,4,5,6,13,14,15,16,17,18) OR p.equip2 = pl.equip AND j.num IN (7,8,9,10,11,12,19,20,21,22,23,24)) AND pl.equip = ` + teamid + `
            GROUP BY pl.nom
